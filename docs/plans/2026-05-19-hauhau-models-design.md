...
Add, benchmark, and install shortcuts for two Hauhau model families on `ubt26`:
...
Qwen and Gemma repos include `mmproj` files. Include mmproj only if smoke/benchmark starts can fit on hardware.
...
- New launchers: `scripts/start12.sh`, `scripts/start14.sh`
