...
def run_diffusion_prompt(
    ...
    """Run DiffusionGemma prompt via SSH. Returns (output_text, return_code).

    If on_line is provided, it's called with each non-step line to support streaming.
    """
    import shlex

    safe_prompt = shlex.quote(prompt)
    cmd = (
        f"HIP_VISIBLE_DEVICES=0,1 ROCR_VISIBLE_DEVICES=0,1 "
        f"{cli} "
        f"-m {model} "
        f"--split-mode layer -ts 1,1 -ngl 99 "
        f"-n {n} --diffusion-steps {steps} --diffusion-visual "
        f"-p {safe_prompt}"
    )
    ...
    try:
        proc = subprocess.Popen(  # noqa: S603 # nosec: B603
            [SSH_BIN, "-o", "BatchMode=yes", "-o", "ConnectTimeout=10", host, cmd],
            stdout=subprocess.PIPE,
        ...
        for line in proc.stdout:
            ...
            if step_pat.search(line):
                continue
            if re.match(r"^\d+\.\d+\.\d+\.\d+ [WIEDS] ", line):
                continue
            if not line:
                continue
