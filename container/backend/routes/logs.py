...
@router.get("/stream")
async def stream_logs(request: Request, no_history: bool = Query(default=False)):
    """SSE stream of model.log tail."""

    async def event_generator():
        # Send buffered history first
        if not no_history:
            for line in read_log_tail(50):
        ...
        try:
            async for sse_chunk in stream_log_tail(disconnect):
